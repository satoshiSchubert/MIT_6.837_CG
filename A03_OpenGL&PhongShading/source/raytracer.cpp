...
void render_forge() {}
// Constructor.
//rayTracer::rayTracer(char* input_file, char* output_file, char* depth_file, char* norm_file, int width, int height, float depth_min, float depth_max, bool SHADE_BACK = false, bool gui = false, int theta_steps=5, int phi_steps=5, bool gouraud=false){
rayTracer::rayTracer(char* input_file, char* output_file, char* depth_file, char* norm_file, int width, int height, float depth_min, float depth_max, bool SHADE_BACK = false, bool gui = false) {
	_width = width;
	_height = height;
	...
	if (gui) {
		canvas = new GLCanvas();
		canvas->initialize(scene, render_forge);// HACK:need modify
		return;
	}	
}
